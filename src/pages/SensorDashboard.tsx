...
const SensorDashboard: React.FC = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const tempResponse = await axios.get<TemperatureData>('/api/temperatura');
                const humidityResponse = await axios.get<HumidityData[]>('/api/humedad');
                const soilMoistureResponse = await axios.get<SoilMoistureData[]>('/api/humedad_tierra');                
